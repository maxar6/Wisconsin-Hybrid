...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_DynoController_Rev1_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_DynoController_Rev1_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_DynoController_Rev1_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "DynoController_Rev1") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_DynoController_Rev1_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_DynoController_Rev1_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"DynoController_Rev1")) {
    return(sf_DynoController_Rev1_method_dispatcher(simstructPtr,chartFileNumber,
...
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    DynoController_Rev1_terminator();
    sf_debug_terminate();
  }
...
extern void DynoController_Rev1_register_exported_symbols(SimStruct* S);
extern void DynoController_Rev1_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  DynoController_Rev1_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      DynoController_Rev1_debug_initialize();
    }

    DynoController_Rev1_initializer();
  }
