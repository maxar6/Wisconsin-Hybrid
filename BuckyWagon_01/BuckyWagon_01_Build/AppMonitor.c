...
/* Name: SHUTDOWN_EVENT_RunCnt ClassID:VAR StorageID:RAM Access:RW4+RW3+NA2+NA1 TypeID:SINT4 CType:int32_T Struct:SHUTDOWN_EVENT_RunCnt Gain:1 Min:-1 Max:2147483646
   Format:5.0 UpdateID:BACKGND Group:"System | Debug | Event Pause Counters" Help:"The number of times to execute SHUTDOWN_EVENT before pausing.  -1 means run forever" */
int32_T SHUTDOWN_EVENT_RunCnt = -1;
...
void StepApplicationStartup(void)
{
  ...
  if ((ApplicationInitStatus == APP_INIT_BEFORE_MODEL) && (ApplicationPausePoint
    ...
    /* Initialize Application Model: BuckyWagon_01 */
    BuckyWagon_01_initialize(1);
    BuckyWagon_01_step();
