...
/*
   ...
   masked_id = 0x18EF0232

   Masks: CAN_1 (ext)
   computed mask = 0x1FFFFFFF
   computed id   = 0x18EF0232
...
} MHCAN_queue_CCP_BAT = { 0 };

uint32_T RxSlot_CCP_BAT_ID = 0;
uint32_T RxSlot_CCP_BAT_IDMask = 0;
static boolean_T MHCAN_handler_CCP_BAT(S_CANMessage *messageObj)
{
  if ((messageObj->u4MessageID & 0x1FFFFFFF) == 0x18EF0232) {
    if ((messageObj->u4MessageID & RxSlot_CCP_BAT_IDMask) == (RxSlot_CCP_BAT_ID
         & RxSlot_CCP_BAT_IDMask)) {
      boolean_T good_payload = 1;
      messageObj->u1Length = messageObj->u1Length > 8 ? 8 : messageObj->u1Length;
      if (good_payload) {
        boolean_T new_message;
        new_message = MHCAN_enqueue((MHCAN_queue *) &MHCAN_queue_CCP_BAT, 4,
          messageObj);
      }
...
static const MHCAN_id_dispatcher_T MHCAN_consumers_0x1FFFFFFF[] = {
  { 0x18ef0232, MHCAN_handler_CCP_BAT },
};
