...
void StepApplicationStartup(void)
{
  ...
  if ((ApplicationInitStatus == APP_INIT_BEFORE_MODEL) && (ApplicationPausePoint
    ...
    /* Initialize Application Model: BatteryController_Rev1 */
    BatteryController_Rev1_initialize(1);
    BatteryController_Rev1_step();
...
void CheckApplicationStatus(void)
{
  ...
  if (stopApplication) {
    ...
    {
      /* Turn off PWM output */
      extern void EST1_PWMOutput_PWMOutput_Stop(void);
      EST1_PWMOutput_PWMOutput_Stop();
    }
  } else {
