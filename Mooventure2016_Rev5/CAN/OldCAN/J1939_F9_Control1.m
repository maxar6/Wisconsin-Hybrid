function msg = motohawk_can_example()
...
%%
  ...
  msg.name                    = 'F9_Control1';
  msg.description             = 'OP_Mode,DES_RPM,DES_NM,DES_Bus_V';
  msg.protocol                = 'J1939';
  msg.module                  = 'PCM-1';
  msg.bus_name                = 'CAN_1';

  msg.idext                   = 'EXTENDED';
  msg.id                      = hex2dec('00ffa600');
  msg.idmask                  = hex2dec('00ffff00');
  msg.idinherit               =  0;
  msg.payload_size            =  8;
  msg.payload_value           = [];
  msg.payload_mask            = [];
  ...
  msg.fields{field_ctr}.name          = 'OP_Mode';
  ...
  %%msg.fields{field_ctr}.maximum       = 2^11-1;  %% Defaults to maximum possible value in field
    ...
    field_ctr = field_ctr + 1;
  msg.fields{field_ctr}.name          = 'Desired_RPM';
  ...
  msg.fields{field_ctr}.offset        = 0;
  %%msg.fields{field_ctr}.minimum       = -(2^11); %% Defaults to minimum possible value in field
  %%msg.fields{field_ctr}.maximum       = 2^11-1;  %% Defaults to maximum
  %%possible value in field
  
    field_ctr = field_ctr + 1;
  msg.fields{field_ctr}.name          = 'Desired_Torque';
  ...
  msg.fields{field_ctr}.offset        = -3200;
  %%msg.fields{field_ctr}.minimum       = -(2^11); %% Defaults to minimum possible value in field
  %%msg.fields{field_ctr}.maximum       = 2^11-1;  %% Defaults to maximum
  %%possible value in field
  ...
  field_ctr = field_ctr + 1;
  msg.fields{field_ctr}.name          = 'Desired_Bus_Voltage';
  msg.fields{field_ctr}.units         = 'Volts';
