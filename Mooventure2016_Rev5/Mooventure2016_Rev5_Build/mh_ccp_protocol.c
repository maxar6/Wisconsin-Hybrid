...
extern S_EriReqTblList const* GetERIRequestTablePtr(NativeVar_U uTableIndex);

/*---- TYPEDEF ----------------------------------------------------------------------------------------------*/
typedef enum E_Exceptions {
  EXCEPTION_NONE = 0,
  EXCEPTION_MEMORY_EXCEPTION = 1,
} E_Exceptions;

typedef void (*MemFuncCBackPtr)(void const* pResultBuffer, uint4 uBytesAffected,
  E_Exceptions eException, NativePtrSizedInt_U uAppDataSentOnNotify);
...
/*---- CODE SPACE -----------------------------------------------------------------------------------------*/
static boolean_T MemWrite(void *AddrToWrite, void const* pDataBuffer, uint32_T
  in_uNumBytesToWrite, MemFuncCBackPtr pfNotifyOnWriteComplete,
  NativePtrSizedInt_U uAppDataSendOnNotify)
{
  E_Exceptions eWriteException = EXCEPTION_NONE;

  /* We access our memory synchronously so the callback shall be called directly after we have
     accessed the memory */
  NativeError_S sError = Memory_WriteSynch(AddrToWrite,pDataBuffer,(NativeVar_U)
    in_uNumBytesToWrite);
  if (FAILED(sError)) {
    eWriteException = EXCEPTION_MEMORY_EXCEPTION;
  }

  if (pfNotifyOnWriteComplete) {
    pfNotifyOnWriteComplete(pDataBuffer, in_uNumBytesToWrite, eWriteException,
      uAppDataSendOnNotify);
  }

  return true;
}

static boolean_T MemRead(void *AddrToRead, uint32_T in_uNumBytesToRead,
  MemFuncCBackPtr pfNotifyOnReadComplete, NativePtrSizedInt_U
  uAppDataSendOnNotify)
{
  E_Exceptions eReadException = EXCEPTION_NONE;
  uint8_T u8DataArr[CCP_WRITE_BUF_SIZE];

  /* We access our memory synchronously so the callback shall be called directly after we have
     accessed the memory */
  NativeError_S sError = SUCCESS_OK;
  if (in_uNumBytesToRead > CCP_WRITE_BUF_SIZE ) {
    eReadException = EXCEPTION_MEMORY_EXCEPTION;
  } else {
    sError = Memory_ReadSynch(u8DataArr,AddrToRead,(NativeVar_U)
      in_uNumBytesToRead);
  }
  ...
  if (pfNotifyOnReadComplete != NULL) {
    pfNotifyOnReadComplete(u8DataArr, in_uNumBytesToRead, eReadException,
      uAppDataSendOnNotify);
  }
...
static void InitialiseCRC32(uint4 * crc_ptr)
{
  BASEPTRARR pvPointerArr = GetModuleConfigPointerArr();

  /* Table revision is ALWAYS entry zero and points to an int. */
  int32_T const* __FAR_POINTER ps32Version = (int32_T const* __FAR_POINTER)
    pvPointerArr[0];
  ...
  if (((uint32_T)ps32Version) != 0xFFFFFFFFL) {
    switch (*ps32Version)
    {
     case PV1:
      {
        /* Table is a revision one */
        ((PFN_CRC32_INITIALIZE) pvPointerArr[MC_PV1_CRC32_INITIALIZE])(crc_ptr);
      }
...
static void AccumulateCRC32(uint4 * crc_ptr, uint1 * byte_ptr, uint4 length)
{
  BASEPTRARR pvPointerArr = GetModuleConfigPointerArr();
  ...
  if (((uint32_T)ps32Version) != 0xFFFFFFFFL) {
    switch (*ps32Version)
    {
     case PV1:
      {
        /* Table is a revision one */
        ((PFN_CRC32_ACCUMULATE) pvPointerArr[MC_PV1_CRC32_ACCUMULATE])(crc_ptr,
          byte_ptr, length);
      }
...
static uint4 ReturnCRC32Value(uint4 * crc_ptr)
{
  BASEPTRARR pvPointerArr = GetModuleConfigPointerArr();
  ...
  if (((uint32_T)ps32Version) != 0xFFFFFFFFL) {
    switch (*ps32Version)
    {
     case PV1:
      {
        /* Table is a revision one */
        return ((PFN_CRC32_RETURNVALUE) pvPointerArr[MC_PV1_CRC32_RETURNVALUE])
          (crc_ptr);
      }
        ...
        if (CCPHandlerConfig->bFrameNotReady[DAQidx] == FALSE) {
          ...
          for (; uPid <= uLastPid; uPid++) {
            ...
            for (i=0; i<nODTSITEMSPERLIST; i++) {
              ...
              Memory_ReadSynch(&data[bytesThisMessage + 1], item->pData,
                               item->DataSize);
...
/*=========================================================================================================*/
{
  ...
  while ((BytesRemaining > 0) && pMTA->Valid && (uTime_us < 250000UL)) {
    ...
    /* Further limit the bytes to read based on the write buffer size */
    BytesToRead = CCP_WRITE_BUF_SIZE < BytesToRead ? CCP_WRITE_BUF_SIZE :
      BytesToRead;
    Memory_ReadSynch(CCPReplyStruct->TransferAddress, src, (NativeVar_U)
                     BytesToRead);
    AccumulateCRC32((uint32_T*)&CCPReplyStruct->TransmitDataArr[4],
                    CCPReplyStruct->TransferAddress, BytesToRead);
