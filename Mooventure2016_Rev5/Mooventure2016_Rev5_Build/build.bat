...
@if ERRORLEVEL 1 exit junk
@"c:\program files (x86)\woodward\mcs\motohawk\2011a_sp0.184\motocoder\bin\nant\bin\nant" -buildfile:TDB\Database.build -D:required.installdir="C:\Program Files (x86)\Woodward\DevelopmentTools\Toolchains\GCC\win32-pe\4_4_0" -D:database.basename=Mooventur_206 -q -nologo rebuild
@if ERRORLEVEL 1 exit junk
...
@echo ### Copy Mooventur_206.dll and Mooventure2016_Rev5_206.srz
@if exist C:\ECUFiles\TDBDLL\*.* copy /Y .\TDB\Mooventur_206.dll C:\ECUFiles\TDBDLL > NUL
@if ERRORLEVEL 1 exit junk
@if exist C:\ECUFiles\Programs\*.* copy /Y .\Target\Mooventure2016_Rev5.srz C:\ECUFiles\Programs\Mooventure2016_Rev5_206.srz > NUL
@if ERRORLEVEL 1 exit junk
