...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_cx06_MA_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_cx06_MA_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_cx06_MA_autoinheritance_info( int nlhs, mxArray * plhs[],
    int nrhs, const mxArray * prhs[] );
  if (sf_cx06_MA_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_cx06_MA_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "cx06_MA") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_cx06_MA_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_cx06_MA_method_dispatcher(SimStruct *S, unsigned int
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"cx06_MA")) {
    return(sf_cx06_MA_method_dispatcher(simstructPtr,chartFileNumber,specsCksum,
            method,data));
  }
...
extern void cx06_MA_terminator(void);
...
extern void cx06_MA_initializer(void);
extern void cx06_MA_register_exported_symbols(SimStruct* S);
extern void cx06_MA_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  cx06_MA_register_exported_symbols(S);
}
